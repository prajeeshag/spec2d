...
#include "netcdf.inc"
  ...
  namelist /regrid_nml/ num_flds, src_data, dst_data, grid_spec_file, fld_name, &
       vector_fld, num_nbrs, max_dist, debug
...
contains
  ...
  subroutine read_xgrid()
    ...
    rcode = nf_get_att_int(ncid,NF_GLOBAL,'maxlon',maxlon)
    call error_handler('error in inquring maxlon value of file ' &
                              //trim(grid_spec_file), rcode )

    rcode = nf_get_att_int(ncid,NF_GLOBAL,'ocnx',ocnx)
    call error_handler('error in inquring ocnx value of file ' &
                              //trim(grid_spec_file), rcode )

    rcode = nf_get_att_int(ncid,NF_GLOBAL,'ocny',ocny)
    call error_handler('error in inquring ocny value of file ' &
                              //trim(grid_spec_file), rcode )

    rcode = nf_get_att_int(ncid,NF_GLOBAL,'nlat',nlat)
    call error_handler('error in inquring nlat value of file ' &
                              //trim(grid_spec_file), rcode )

    !--- get exchange grid information
    ncells = get_dimlen(ncid, 'xn')
    allocate(rxi(ncells), rxj(ncells), &
             pxi(ncells), pxj(ncells), &
             xarea(ncells))

    call get_var_int_1d (ncid, 'rxi', rxi)
    call get_var_int_1d (ncid, 'pxi', pxi)
    call get_var_int_1d (ncid, 'rxj', rxj)
    call get_var_int_1d (ncid, 'pxj', pxj)
    call get_var_real_1d(ncid, 'xf',  xarea)

    rcode = nf_close(ncid)
    !--- this variable will be used for the conservative interpolation.
    allocate(area(nlat, maxlon))
  ...
  subroutine setup_meta
    ...
    rcode = nf_inq(ncid_src, ndims, nvars, natt, recdim)
    call error_handler('error in inquring file '//trim(src_data), rcode)
    ...
    do i = 1, natt
       ...
       rcode = nf_copy_att(ncid_src, NF_GLOBAL, trim(name), ncid_dst, NF_GLOBAL)
       call error_handler('error in copying attribute '//trim(name), rcode)
    enddo

    !------ define dimensions 
    do i = 1, ndims
        rcode = nf_inq_dim(ncid_src, i, name, len)
        call error_handler('error in inquring dimension of file '//trim(src_data), rcode)
        if(lowercase(trim(name))=='time' ) then
           rcode = nf_def_dim(ncid_dst, 'time', NF_UNLIMITED, dim_time_dst)
           call error_handler('error in defining time dimension of file '//trim(dst_data), rcode)
           rcode = nf_def_var(ncid_dst, 'time', NF_DOUBLE, 1, timdimid, id_time_dst)
           call error_handler('error in defining time variable of file '//trim(dst_data), rcode)
           cycle 
        endif
        rcode = nf_inq_varid(ncid_src, trim(name), dimvarid)
        call error_handler('error in inquring dimension variable '//trim(name), rcode)
        rcode = nf_inq_att(
  ...
  subroutine process_data
    ...
    do m = 1, ntime_src
       ...
       do while ( n .le. num_flds )
          ...
          allocate(data_src(nk, ocny, ocnx, nf), data_dst(nk, nlat, maxlon, nf) )
          ...
          do l = 1, nf
             !---- doing horizontal interpolaiton
                call conserve_interp(data_src(:,:,:,1), data_dst(:,:,:,l), missing_value(1))
          enddo
