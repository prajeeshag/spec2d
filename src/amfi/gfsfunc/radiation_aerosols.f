...
!........................................!
!
      use physcons_mod, only : con_pi, con_rd, con_fvirt, con_g,        
...
!      sigref(NDM,NAE)  - ref sigma lev (sfc to toa)
      ...
      real , allocatable, save, dimension(:,:) :: haer
      real , allocatable, save, dimension(:,:) :: prsref
      real , allocatable, save, dimension(:,:) :: sigref
...
!      denng (NXC*IMXAE*JMXAE)   - aerosols number density
     ...
     &       extstra
      ...
      real ,allocatable,save:: cmixg(:,:,:),denng(:,:,:)
...
!  ---  locals:
      real , dimension(NWVTOT) :: solfwv        ! one wvn sol flux
      real , dimension(NWVTIR) :: eirfwv        ! one wvn ir flux
      real  :: soltot, tmp1, tmp2, tmp3
...
!  ---  inputs:
      ...
      real , dimension(:,:), intent(in) :: prsi, prsl,  
     &       tlay, qlay, rhlay
...
!  ---  locals:
      real , dimension(IMAX) :: alon, alat, volcae, delp
      real  :: prsln(NLP1),hz(IMAX,NLP1),dz(IMAX,NLAY)
      real  :: tmp1, tmp2, psrfh, psrfl
...
!  ---  conversion constants
      real , parameter :: rdg  = 180.0 / con_pi
      real , parameter :: rovg = 0.001 * con_rd / con_g
...
!  ---  inputs:
      ...
      real , intent(in) :: solfwv(:),soltot, eirfwv(:)

!  ---  output: ( none )

!  ---  locals:
      real :: cmix(NXC), denn, tem
      integer              :: idxc(NXC), kprf

      integer, dimension(NAERBND) :: iendwv

      real , dimension(NAERBND,NCM1)       ::           
     &       rhidext0, rhidsca0, rhidssa0, rhidasy0
      real , dimension(NAERBND,NRHLEV,NCM2)::           
     &       rhdpext0, rhdpsca0, rhdpssa0, rhdpasy0
      real , dimension(NAERBND)            :: straext0

      real , dimension(NBDSW,NAERBND) :: solwaer
      real , dimension(NBDSW)         :: solbnd
      real , dimension(NBDIR,NAERBND) :: eirwaer
      real , dimension(NBDIR)         :: eirbnd
      real  :: sumsol, sumir
...
!  ---  locals:
      real  :: sumk, sums, sumok, sumokg, sumreft,      
...
!  ---  inputs:
      ...
      real , dimension(:,:), intent(in) :: prsi,        
     &       rhlay, dz, hz
      real , dimension(:),   intent(in) :: alon, alat
...
!  ---  locals:
      real , dimension(NXC)  :: cmix, denn
      integer,               dimension(NXC)  :: idxc

      real , dimension(NLAY) :: delz, rh1, dz1
      integer,               dimension(NLAY) :: idmaer

      real , dimension(NLAY,NBDSWLW):: tauae,ssaae,asyae
!test real , dimension(IMAX,NLAY) :: aersav
...
!  ---  conversion constants
      real , parameter :: dltg = 360.0 / float(IMXAE)
      real , parameter :: hdlt = 0.5 * dltg
...
!  ---  locals:
      real  :: cm, hd, hdi, sig0u, sig0l, ratio, tt0,   
