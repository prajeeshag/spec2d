...
module spectral_dynamics_mod
  ...
  use constants_mod, only : RVGAS, RDGAS, GRAV, RADIUS, HLV, CP_AIR
  ...
  real, parameter :: cont = 1000.*CP_AIR/GRAV, &
    conq = 1000.*HLV/GRAV, con_ke = 0.5*1000./GRAV
  ...
  integer :: commID
  real :: deltim=600., rdeltim=1./600.
  real :: filta = 0.85 
  logical :: mass_corr=.true.
  logical :: ene_corr=.true., do_energy_correction=.true.
  logical :: zero_topo=.false., debug=.false.
  integer :: niter_topo = 0
  real :: total_energy_prev, mean_psurf_prev
  ...
  interface calc_total_energy
    module procedure calc_total_energy1
    module procedure calc_total_energy2
  end interface calc_total_energy
  ...
  contains
    ...
    !--------------------------------------------------------------------------------   
      ...
      namelist/spectral_dynamics_nml/zero_topo, topo_file, topo_field, pdryini, debug
    ...
    !--------------------------------------------------------------------------------
      ...
      real :: val, ene1, ene2, inte1, inte2, kine1, kine2, rdeltim2
      integer :: i, j, k, ntr
      logical :: used
      character (len=512) :: msg

      if (.not.Initialized) call mpp_error(FATAL,'spectral_dynamics_mod: Not Initialized')
      ...
      call compute_corrections(u1,v1,tem1,p1)

    end subroutine spectral_dynamics


    !--------------------------------------------------------------------------------   
    subroutine finish_spectral_dynamics(Time, tem, tr, u, v, ene)
    !--------------------------------------------------------------------------------   
      type(time_type), intent(in) :: Time
      real, intent(in), dimension(nlev,jsp:jep,isp:iep)       :: u, v, tem
      real, intent(in), dimension(nlev,jsp:jep,isp:iep,ntrac) :: tr
      real, intent(in), optional, dimension(jsp:jep,isp:iep) :: ene

      complex, dimension(nlev,nwaves_oe,2) :: rqt
      real, dimension(nlev,jsp:jep,isp:iep) :: gtmp1
      real, dimension(jsp:jep,isp:iep) :: ene1
    ...
    !-------------------------------------------------------------------------------- 
    subroutine initialize_corrections()
    !-------------------------------------------------------------------------------- 
      real, dimension(nlev,jsp:jep,isp:iep) :: u, v, div, temp, qr, delp
      real, dimension(nlev+1,jsp:jep,isp:iep) :: prsi
      real, dimension(1,jsp:jep,isp:iep) :: ps
      real, dimension(jsp:jep,isp:iep) :: heatc, kinetic, tote, psurf
      real :: rtote, gtote
      integer :: k

      if (.not.do_energy_correction) return

      call compute_ucos_vcos(satm(1)%vor,satm(1)%div,sucos,svcos,do_trunc=.false.)
      call spherical_to_grid(sucos,grid=u)
      call spherical_to_grid(svcos,grid=v)
      do k = 1, size(u,1)
        u(k,jsp:jep,isp:iep) = u(k,jsp:jep,isp:iep) * cosm_latP(jsp:jep,isp:iep)
        v(k,jsp:jep,isp:iep) = v(k,jsp:jep,isp:iep) * cosm_latP(jsp:jep,isp:iep)
      enddo

      call spherical_to_grid(satm(1)%tem,grid=temp)

      call spherical_to_grid(satm(1)%tr(:,:,:,1),grid=qr)
      where(qr<0.) qr=0.

      call spherical_to_grid(satm(1)%prs,grid=ps)
      ps = exp(ps)

      temp(:,jsp:jep,isp:iep) = temp(:,jsp:jep,isp:iep) / &
        (1.0+fv*qr(:,jsp:jep,isp:iep))

      call get_pressure_at_levels(ps(1,:,:),prsi=prsi)
      delp(1:nlev,jsp:jep,isp:iep) = prsi(1:nlev,jsp:jep,isp:iep) - prsi(2:nlev+1,jsp:jep,isp:iep)
    
      heatc = 0.
      kinetic = 0.
      do k = 1, nlev
        heatc(jsp:jep,isp:iep) = cont*delp(k,jsp:jep,isp:iep)*temp(k,jsp:jep,isp:iep)
        kinetic(jsp:jep,isp:iep) = con_ke*delp(k,jsp:jep,isp:iep) * &
          (u(k,jsp:jep,isp:iep)*u(k,jsp:jep,isp:iep)+v(k,jsp:jep,isp:iep)*v(k,jsp:jep,isp:iep))
      end do

      tote(jsp:jep,isp:iep) = kinetic(jsp:jep,isp:iep) + heatc(jsp:jep,isp:iep)
      tote(jsp:jep,isp:iep) = tote(jsp:jep,isp:iep) * wtsbynlon(jsp:jep,isp:iep)*0.5
      total_energy_prev = sum(tote(jsp:jep,isp:iep))
      call mpp_sum(total_energy_prev)

      psurf(jsp:jep,isp:iep) = ps(1,jsp:jep,isp:iep) * wtsbynlon(jsp:jep,isp:iep)*0.5
      mean_psurf_prev = sum(psurf(jsp:jep,isp:iep))
      call mpp_sum(mean_psurf_prev)
    ...
    !-------------------------------------------------------------------------------- 
    subroutine compute_corrections(u,v,temp,ps)
    !-------------------------------------------------------------------------------- 
      real, intent(in), dimension(1:,jsp:,isp:) :: u, v
      real, intent(in), dimension(jsp:,isp:) :: ps
      real, intent(inout), dimension(1:,jsp:,isp:) :: temp
      !local
      real, dimension(size(u,1),jsp:jep,isp:iep) :: delp
      real, dimension(size(v,1)+1,jsp:jep,isp:iep) :: prsi
      real, dimension(jsp:jep,isp:iep) :: heatc, kinetic, tote, psurf
      real :: rtote, gtote, totenergy, mean_psurf, temp_corr
      integer :: k

      if (.not.do_energy_correction) return

      call get_pressure_at_levels(ps(:,:),prsi=prsi)
      delp(1:nlev,jsp:jep,isp:iep) = prsi(1:nlev,jsp:jep,isp:iep) - prsi(2:nlev+1,jsp:jep,isp:iep)
      ...
      tote(jsp:jep,isp:iep) = kinetic(jsp:jep,isp:iep) + heatc(jsp:jep,isp:iep)
      tote(jsp:jep,isp:iep) = tote(jsp:jep,isp:iep) * wtsbynlon(jsp:jep,isp:iep)*0.5
      totenergy = sum(tote(jsp:jep,isp:iep))

      call mpp_sum(totenergy)

      temp_corr = GRAV*(total_energy_prev - totenergy)/(CP_AIR*mean_psurf_prev*1000.)

      if (debug) then
        if (mpp_pe()==mpp_root_pe()) then
          print *, 'Energy error (W/m2) from spect dyn : ', (total_energy_prev - totenergy)/(2.*deltim)
    ...
    !-------------------------------------------------------------------------------- 
    subroutine calc_total_energy1(u,v,temp,qr,ps,totenergy,inte,kine,lat)
    !-------------------------------------------------------------------------------- 
      real, intent(in), dimension(1:,jsp:,isp:) :: u, v, temp, qr, ps
      real, intent(out) :: totenergy
      real, intent(out), optional :: lat, inte, kine
      !local
      real, dimension(size(u,1),jsp:jep,isp:iep) :: delp
      real, dimension(size(v,1)+1,jsp:jep,isp:iep) :: prsi
      real, dimension(jsp:jep,isp:iep) :: heatc, moistc, kinetic, tote
      real :: rtote, gtote
      integer :: k, nlev
      ...
      kinetic = 0.
      do k = 1, nlev
        heatc(jsp:jep,isp:iep) = heatc(jsp:jep,isp:iep)+cont*delp(k,jsp:jep,isp:iep)*temp(k,jsp:jep,isp:iep)
        moistc(jsp:jep,isp:iep) = moistc(jsp:jep,isp:iep)+conq*delp(k,jsp:jep,isp:iep)*qr(k,jsp:jep,isp:iep)
        kinetic(jsp:jep,isp:iep) = kinetic(jsp:jep,isp:iep)+con_ke*delp(k,jsp:jep,isp:iep) * &
          (u(k,jsp:jep,isp:iep)*u(k,jsp:jep,isp:iep)+v(k,jsp:jep,isp:iep)*v(k,jsp:jep,isp:iep))
      end do
    ...
    end subroutine calc_total_energy1


    !-------------------------------------------------------------------------------- 
    subroutine calc_total_energy2(svor,sdiv,stemp,sqr,sps,totenergy,inte,kine,late)
    !-------------------------------------------------------------------------------- 
      complex, dimension(:,:,:), intent(in) :: svor, sdiv, stemp, sqr, sps
      real, intent(out) :: totenergy
      real, intent(out), optional :: late, inte, kine
      !local
      complex, dimension(size(svor,1),size(svor,2),size(svor,3)) :: sucos, svcos
      real, dimension(size(svor,1),jsp:jep,isp:iep) :: u, v, div, temp, qr, delp
      real, dimension(size(svor,1)+1,jsp:jep,isp:iep) :: prsi
      real, dimension(size(sps,1),jsp:jep,isp:iep) :: ps
      real, dimension(jsp:jep,isp:iep) :: heatc, moistc, kinetic, tote
      real :: rtote, gtote
      integer :: k, nlev
      ...
      call compute_ucos_vcos(svor,sdiv,sucos,svcos,do_trunc=.false.)
      call spherical_to_grid(sucos,grid=u)
      call spherical_to_grid(svcos,grid=v)
      do k = 1, size(u,1)
        u(k,jsp:jep,isp:iep) = u(k,jsp:jep,isp:iep) * cosm_latP(jsp:jep,isp:iep)
      ...
      call spherical_to_grid(stemp,grid=temp)

      call spherical_to_grid(sqr,grid=qr)
      where(qr<0.) qr=0.

      call spherical_to_grid(sps,grid=ps)
      ps = exp(ps)
      ...
      !tote(jsp:jep,isp:iep) = kinetic(jsp:jep,isp:iep) + moistc(jsp:jep,isp:iep) + heatc(jsp:jep,isp:iep)
      tote(jsp:jep,isp:iep) = kinetic(jsp:jep,isp:iep) + heatc(jsp:jep,isp:iep)
      ...
      if (present(inte)) then
        heatc(jsp:jep,isp:iep) = heatc(jsp:jep,isp:iep)* wtsbynlon(jsp:jep,isp:iep)*0.5
        inte = sum(heatc(jsp:jep,isp:iep))
        call mpp_sum(inte)
      end if

      if (present(kine)) then
        kinetic(jsp:jep,isp:iep) = kinetic(jsp:jep,isp:iep)* wtsbynlon(jsp:jep,isp:iep)*0.5
        kine = sum(kinetic(jsp:jep,isp:iep))
        call mpp_sum(kine)
      end if

      if (present(late)) then
        moistc(jsp:jep,isp:iep) = moistc(jsp:jep,isp:iep)* wtsbynlon(jsp:jep,isp:iep)*0.5
        late = sum(moistc(jsp:jep,isp:iep))
        call mpp_sum(late)
      end if

      return

    end subroutine calc_total_energy2

    !-------------------------------------------------------------------------------- 
    subroutine get_gtopo(topo)
    !-------------------------------------------------------------------------------- 
      real, intent(out) :: topo(jsp:jep,isp:iep)

      if (.not.Initialized) call mpp_error(FATAL,'spectral_dynamics_mod: Not Initialized')
      topo(jsp:jep,isp:iep) = gtopo(1,jsp:jep,isp:iep)

      return
    end subroutine get_gtopo

    !-------------------------------------------------------------------------------- 
    subroutine get_cellareawts(wts)
    !-------------------------------------------------------------------------------- 
      real, intent(out) :: wts(jsp:jep,isp:iep)

      if (.not.Initialized) call mpp_error(FATAL,'spectral_dynamics_mod: Not Initialized')
      wts(jsp:jep,isp:iep) = wtsbynlon(jsp:jep,isp:iep)*0.5

      return
    end subroutine get_cellareawts
