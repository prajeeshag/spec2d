        module gwdc_mod
        implicit none
         integer, parameter, private :: im=1,ix=1,iy=1
        contains 
        subroutine gwdc(km,u1,v1,t1,q1,
     &                pmid1,pint1,dpmid1,qmax,cumchr1,ktop,kbot,kuo,
     &                fu1,fv1,g,cp,rd,fv,dlength,tauctx,taucty)
...
!-----------------------------------------------------------------------
      ...
      real g,cp,rd,fv,dlength(im)
      real qmax(ix),cumchr1(ix,km),cumchr(ix,km)
      real fhour,fhourpr
      real u1(ix,km),v1(ix,km),t1(ix,km),q1(ix,km),
     &                     pmid1(ix,km),dpmid1(ix,km),pint1(ix,km+1),
     &                     fu1(iy,km),fv1(iy,km)
      real u(im,km),v(im,km),t(im,km),spfh(im,km),
...
!-----------------------------------------------------------------------
     ...
     &     riloc(km+1),rimin(km+1)
      ...
      real gwdcloc(im),break(im),critic(im)
      real tem1, tem2, qtem
...
!-----------------------------------------------------------------------
      ...
      real ucltop, vcltop, windcltop, shear, kcldtopi
      real cosphi, sinphi, angle
      real nonlinct, nonlin, nonlins, taus 
...
!-----------------------------------------------------------------------
     ...
     &,                     riminp=-0.99e+20, rismall=-0.9e+20
      ...
      real n2, dtdp, sum1, xstress, ystress
      real crit1, crit2
      real pi,p1,p2
...
!        Make regular (U,V) by using array 
...
!        Make output Tendencies cos-weighted by using array 
