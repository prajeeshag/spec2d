...
!     sh2o     - real, unfrozen soil moisture (vol fraction)     :
...
!  ====================    end of description    =====================  !
!
      use constants_mod, only : con_cp => CP_AIR, con_rd => RDGAS, 
     &    con_t0c => KELVIN, con_g => GRAV, con_pi => PI,    
     &    con_cliq => CP_WATER, con_csol => CP_ICE, 
     &    con_sbc => STEFAN
!
      implicit none
...
!          provide the same result as the original codes.  -- y.t.h.  may09
      ...
      real , parameter :: gs      = con_g       ! con_g   =9.80665
      real , parameter :: gs1     = 9.8         ! con_g in sfcdif
      real , parameter :: gs2     = 9.81        ! con_g in snowpack, frh2o
      real , parameter :: tfreez  = con_t0c     ! con_t0c =275.15
      real , parameter :: lsubc   = 2.501e+6    ! con_hvap=2.5000e+6
      real , parameter :: lsubf   = 3.335e5     ! con_hfus=3.3358e+5
      real , parameter :: lsubs   = 2.83e+6     ! ? in sflx, snopac
      real , parameter :: elcp    = 2.4888e+3   ! ? in penman
      real , parameter :: rd      = con_rd      ! con_rd  =287.05
      real , parameter :: rd1     = 287.04      ! con_rd in sflx, penman, canres
      real , parameter :: cp      = con_cp      ! con_cp  =1004.6
      real , parameter :: cp1     = 1004.5      ! con_cp in sflx, canres
      real , parameter :: cp2     = 1004.0      ! con_cp in htr
      real , parameter :: cph2o   = con_cliq    ! con_cliq=4.1855e+3
      real , parameter :: cph2o1  = 4.218e+3    ! con_cliq in penman, snopac
      real , parameter :: cph2o2  = 4.2e6       ! con_cliq in hrt *unit diff!
      real , parameter :: cpice   = con_csol    ! con_csol=2.106e+3
      real , parameter :: cpice1  = 2.106e6     ! con_csol in hrt *unit diff!
      real , parameter :: sigma1   = con_sbc     ! con_sbc=5.6704e-8

!  ---  inputs:
      integer, intent(in) :: nsoil, couple, icein, vegtyp, soiltyp,     
     &       slopetyp

      real , intent(in) :: ffrozp, dt, zlvl, lwdn,      
     &       sldpth(nsoil), swdn, swnet, sfcems, sfcprs, sfctmp,        
     &       sfcspd, prcp, q2, q2sat, dqsdt2, th2

!  ---  input/outputs:
      real , intent(inout) :: tbot, cmc, t1, sneqv,     
     &       stc(nsoil), smc(nsoil), sh2o(nsoil), ch, cm, snowh,
     &       shdfac
...
!  ---  locals:
      real  ::  bexp, cfactr, cmcmax, csoil, czil,      
     ...
     &       t1v, t24, t2v, th2v, topt, tsnow, zbot, z0
      ...
      real , dimension(nsold) :: rtdis, zsoil
...
!  --- ...  determine snowcover fraction over land.

      if (ice /= 0) then

!  --- ...  snow cover over sea-ice, glacial-ice
...
!     real , intent(in) :: swdn, ch, q2, q2sat,         
!    &       dqsdt2, sfctmp, sfcprs, sfcems, smcwlt, smcref, rsmin,     
!    &       rsmax, topt, rgl, hs, xlai, sh2o(nsoil), zsoil(nsoil)

!  ---  outputs:
!     real , intent(out) :: rc, pc, rcs, rct, rcq,      
!    &       rcsoil

!  ---  locals:
      real  :: delta, ff, gx, rr, part(nsold)
...
!     real , intent(in) :: etp, prcp, smcmax,           
...
!  ---  input/outputs:
!     real , intent(inout) :: cmc, t1, stc(nsoil),      
!    &       sh2o(nsoil), tbot

!  ---  outputs:
!     real , intent(out) :: eta, smc(nsoil), ssoil,     
!    &       runoff1, runoff2, runoff3, edir, ec, et(nsoil), ett,       
!    &       beta, drip, dew, flx1, flx3

!  ---  locals:
      real  :: df1, eta1, etp1, prcp1, yy, yynum,       
...
!           effects caused by falling precipitation.
        ...
        if (ffrozp > 0.) rr = rr + cpice*ffrozp/rch
...
!  ---  outputs:
!     real , intent(out) :: cfactr, cmcmax, rsmin,      
...
!  ---  inputs:
!     real ,  intent(in) :: zlvl, z0, t1v, th2v,        
!    &       sfcspd, czil

!  ---  input/outputs:
!     real ,  intent(inout) :: cm, ch

!  ---  locals:
      real  :: zilfc, zu, zt, rdz, cxch, dthv, du2,     
...
!  ---  define local in-line functions:
      ...
      real  :: pslmu, pslms, pslhu, pslhs, zz
      real  :: pspmu, pspms, psphu, psphs, xx, yy
...
!     real , intent(in) :: etp, prcp, smcmax, smcref,   
...
!           snowfall striking the gound is =sfctmp (lowest model level air temp).
      ...
      flx1 = 0.0
      if (ffrozp > 0.) flx1 = cpice * ffrozp * (t1 - sfctmp)
...
!
!  ---  inputs:
!     real, intent(in) :: sfctmp, sn_new

!  ---  input/outputs:
!     real, intent(inout) :: snowh, sndens

!  ---  locals:
      real :: dsnew, snowhc, hnewc, newsnc, tempc
...
!
!  ---  input:
      real , intent(in) :: smc, qz, smcmax, sh2o

!  ---  output:
      real , intent(out) :: df

!  ---  locals:
      real  :: gammd, thkdry, ake, thkice, thko,        
...
!  ---  inputs:
      ...
      real , intent(in) :: smc(nsoil), smcmax, dt, yy,  
     &       zz1, zsoil(nsoil), zbot, psisat, bexp, df1, quartz, csoil

!  ---  input/outputs:
      real , intent(inout) :: stc(nsoil), t1, tbot,     
     &       sh2o(nsoil)
...
!  ---  locals:
      real  :: ai(nsold), bi(nsold), ci(nsold), oldt1,  
...
!  ---  inputs:
      ...
      real ,  intent(in) :: dt, kdt, smcmax, smcwlt,    
     &       cmcmax, prcp1, slope, frzx, bexp, dksat, dwsat, shdfac,    
     &       edir1, ec1, et1(nsoil), zsoil(nsoil)

!  ---  input/outputs:
      real ,  intent(inout) :: cmc, sh2o(nsoil)

!  ---  outputs:
      real ,  intent(out) :: smc(nsoil), runoff1,       
     &       runoff2, runoff3, drip

!  ---  locals:
      real  :: dummy, excess, pcpdrp, rhsct, trhsct,    
...
!  ---  parameter constants:
      ...
      real , parameter :: c2 = 21.0
      real , parameter :: kn = 4000.0
...
!
!  ---  inputs:
      real , intent(in) :: etp1, smc, shdfac, smcmax,   
     &       smcdry, fxexp

!  ---  outputs:
      real , intent(out) :: edir1
...
!  ---  inputs:
      ...
      real ,  intent(in) :: stc(nsoil), smc(nsoil),     
     &       smcmax, zsoil(nsoil), yy, zz1, tbot, zbot, psisat, dt,     
     &       bexp, df1, quartz, csoil

!  ---  input/outputs:
      real ,  intent(inout) :: sh2o(nsoil)

!  ---  outputs:
      real ,  intent(out) :: rhsts(nsoil), ai(nsold),   
     &       bi(nsold), ci(nsold)
...
!  ---  inputs:
      ...
      real , intent(in) :: stc(nsoil), zsoil(nsoil),    
     &       yy, zz1, df1
...
!  ---  locals:
      real  :: ddz, ddz2, denom, dtsdz, dtsdz2,         
     &       hcpct, ssoil, zbot
...
!  ---  inputs:
      ...
      real ,  intent(in) :: stcin(nsoil), dt

!  ---  input/outputs:
      real ,  intent(inout) :: rhsts(nsoil),            
     &      ai(nsold), bi(nsold), ci(nsold)

!  ---  outputs:
      real ,  intent(out) :: stcout(nsoil)

!  ---  locals:
      integer :: k

      real  :: ciin(nsold), rhstsin(nsoil)
...
!  ---  inputs:
      ...
      real ,  dimension(nsoil), intent(in) :: a, b, d

!  ---  input/outputs:
      real ,  dimension(nsoil), intent(inout) :: c

!  ---  outputs:
      real ,  dimension(nsoil), intent(out) :: p, delta
...
!  ---  inputs:
      ...
      real , intent(in) :: tavg, smc, smcmax, psisat,   
...
!  ---  inputs:
      ...
      real , dimension(nsoil), intent(in) :: et,        
     &       sh2o, sh2oa, zsoil, sice

      real , intent(in) :: edir, pcpdrp, dwsat, dksat,  
...
!  ---  locals:
      real  :: acrt, dd, ddt, ddz, ddz2, denom, denom2, 
...
!  ---  input:
      ...
      real , dimension(nsoil), intent(in) :: sh2oin,    
     &       zsoil, sice

      real , intent(in) :: rhsct, dt, smcmax, cmcmax

!  ---  inout/outputs:
      real , intent(inout) :: cmc, rhstt(nsoil),        
     &       ai(nsold), bi(nsold), ci(nsold)

!  ---  outputs:
      real , intent(out) :: sh2oout(nsoil), runoff3,    
     &       smc(nsoil)

!  ---  locals:
      real  :: ciin(nsold), rhsttin(nsoil), ddz, stot,  
     &       wplus
...
!  ---  input:
      ...
      real , intent(in) :: tu, tb, zbot, zsoil(nsoil)
...
!  ---  input:
      ...
      real , intent(in) :: etp1, smcwlt, smcref,        
...
!  ---  output:
      real , dimension(nsoil), intent(out) :: et1

!  ---  locals:
      real  :: denom, etp1a, rtx, sgx, gx(7)
...
!
!  ---  input:
      real , intent(in)  :: smc, smcmax, bexp, dksat,   
     &       dwsat, sicemax

!  ---  output:
      real , intent(out) :: wdf, wcnd

!  ---  locals:
      real  :: expon, factr1, factr2, vkwgt
!
!===> ...  begin here
!
