...
main (int argc, char *argv[])
{
	...
	double *wet, *ocnlonb, *ocnlatb; 
	double *atmlonbe, *atmlatbn, *lndlonbe, *lndlatbn;
	double *atmlonbw, *atmlatbs, *lndlonbw, *lndlatbs;
	double *atmlonc, *atmlatc, *lndlonc, *lndlatc ;  
	...
	if(strcmp(afile+n-3,".nc") == 0) {
		if (!(atmlonbe = get_var2d_field2d(afile, "lonbe", 1, &im, &jm))) {
			fprintf(stderr, "Couldn't get lonbe from %s\n", afile);
			exit(1);
		} 
		else {if(pe == root_pe)printf("Got lonbe from %s\n", afile);}
		if (!(atmlonbw = get_var2d_field2d(afile, "lonbw", 1, &im, &jm))) {
			fprintf(stderr, "Couldn't get lonbw from %s\n", afile);
			exit(1);
		} 
		else {if(pe == root_pe)printf("Got lonbw from %s\n", afile);}
		if (!(atmlatbn = get_var2d_field2d(afile, "latbn", 1, &im, &jm))) {
			fprintf(stderr, "Couldn't get latbn from %s\n", afile);
			exit(1);
		} 
		else {if(pe == root_pe)printf("Got latbn from %s\n", afile);}
		if (!(atmlatbs = get_var2d_field2d(afile, "latbs", 1, &im, &jm))) {
			fprintf(stderr, "Couldn't get latbs from %s\n", afile);
			exit(1);
		} 
		else {if(pe == root_pe)printf("Got latbs from %s\n", afile);}

		IMA = im;
		JMA = jm;

		if (!(atmlonc = get_var2d_field2d(afile, "lonc", 1, &im, &jm))) {
			fprintf(stderr, "Couldn't get lonc from %s\n", afile);
			exit(1);
		} 
		else {if(pe == root_pe) printf("Got lonc from %s\n", afile);}
		if (!(atmlatc = get_var2d_field2d(afile, "latc", 1, &im, &jm))) {
			fprintf(stderr, "Couldn't get latc from %s\n", afile);
			exit(1);
		} 
		else {if(pe == root_pe)printf("Got latc from %s\n", afile);}

	}

	if(pe == root_pe ) {
		printf("\nAtmosphere Longitude Boundaries: ");
		for (i=0;i<IMA;i++) printf(" %g", atmlonbe[i]);
		printf("\n");
		printf("\nAtmosphere Latitude Boundaries: ");
		for (i=0;i<JMA;i++) printf(" %g", atmlatbs[i]);
		printf("\n");
	}
	/*
	 * Read land grid
	...
	if( pe == root_pe) {
		for (i=0;i<=IML;i++) printf(" %g", lndlonbw[i]);
		printf("\n");
		printf("\nLand Latitude Boundaries: ");
		for (i=0;i<=JML;i++) printf(" %g", lndlatbn[i]);
		printf("\n");
	}
	...
	for (na = 0; na< natm_local; na ++) {
		...
		ll.x = atmlonbw[na]; ll.y = atmlatbs[na];
		ur.x = atmlonbe[na]; ur.y = atmlatbn[na];
		atm_area_left[na]   = atm_area[na] = box_area(ll, ur);
		atm_ctrlon[na]      = box_ctrlon(ll, ur, 0.5*(ll.x+ur.x));
		atm_ctrlat[na]      = box_ctrlat(ll, ur);
		lnd_cell_area[na] = atm_area_left[na];
		lnd_cell_ctrlon[na] = atm_ctrlon[na];
		lnd_cell_ctrlat[na] = atm_ctrlat[na];
	}
	 ...
	 */
	{
		...
		for (na = 0; na < natm_local; na ++) {
			...
			atm_ll.x = atmlonbw[na]; atm_ll.y = atmlatbs[na];
			atm_ur.x = atmlonbe[na]; atm_ur.y = atmlatbn[na];


			for (jl=0;jl<JML;jl++) for (il=0;il<IML;il++) {
				VTX lnd_ll, lnd_ur;

				lnd_ll.x = lndlonbw[jl * (IML)+il]; lnd_ll.y = lndlatbs[jl * (IML)+il];
				lnd_ur.x = lndlonbe[jl * (IML)+il]; lnd_ur.y = lndlatbn[jl * (IML)+il];
			...
			if( npes == 1 ) { /* print this information in parallel run is complicated */
				if (ia+1==IMA) printf( "to %6.2f: %d AL x-cells (area=%g); %d AO x-cells (area=%g)\n",
						atmlatc[na], atmXlnd_n, tot_area_AXL,
	...
	if( pe == root_pe ) {
		...
		NC_CALL(nc_def_var(ncid, "xba", NC_INT, 1, &dim_xba, &xba_id));
		NC_CALL(nc_def_dim(ncid, "yba", JMA+1, &dim_yba));
		NC_CALL(nc_def_var(ncid, "yba", NC_INT, 1, &dim_yba, &yba_id));

		NC_CALL(nc_def_dim(ncid, "xta", IMA, &dim_xta));
		NC_CALL(nc_def_var(ncid, "xta", NC_INT, 1, &dim_xta, &xta_id));
		NC_CALL(nc_def_dim(ncid, "yta", JMA, &dim_yta));
		NC_CALL(nc_def_var(ncid, "yta", NC_INT, 1, &dim_yta, &yta_id));
		...
		NC_CALL(nc_def_var(ncid, "xbl", NC_INT, 1, &dim_xbl, &xbl_id));
		NC_CALL(nc_def_dim(ncid, "ybl", JML+1, &dim_ybl));
		NC_CALL(nc_def_var(ncid, "ybl", NC_INT, 1, &dim_ybl, &ybl_id));

		NC_CALL(nc_def_dim(ncid, "xtl", IML, &dim_xtl));
		NC_CALL(nc_def_var(ncid, "xtl", NC_INT, 1, &dim_xtl, &xtl_id));
		NC_CALL(nc_def_dim(ncid, "ytl", JML, &dim_ytl));
		NC_CALL(nc_def_var(ncid, "ytl", NC_INT, 1, &dim_ytl, &ytl_id));
		...
		for (i=0;i<=IMA;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, xba_id, &i, &ii))};
		for (i=0;i<=JMA;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, yba_id, &i, &ii))};

		for (i=0;i<IMA;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, xta_id, &i, &ii))};

		for (i=0;i<JMA;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, yta_id, &i, &ii))};

		for (i=0;i<=IML;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, xbl_id, &i, &ii))};
		for (i=0;i<=JML;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, ybl_id, &i, &ii))};

		for (i=0;i<IML;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, xtl_id, &i, &ii))};

		for (i=0;i<JML;i++){ ii=i; NC_CALL(nc_put_var1_int(ncid, ytl_id, &i, &ii))};
