...
main (int argc, char *argv[])
{
	...
	for (na = 0; na< natm_local; na ++) {
		...
		ll.x = atmlonbe[na]; ll.y = atmlatbs[na];
		ur.x = atmlonbw[na]; ur.y = atmlatbn[na];
	 ...
	 */
	{
		...
		for (na = 0; na < natm_local; na ++) {
			...
			atm_ll.x = atmlonbe[na]; atm_ll.y = atmlatbs[na];
			atm_ur.x = atmlonbw[na]; atm_ur.y = atmlatbn[na];


			for (jl=0;jl<JML;jl++) for (il=0;il<IML;il++) {
				VTX lnd_ll, lnd_ur;

				lnd_ll.x = lndlonbe[jl * (IML+1)+il]; lnd_ll.y = lndlatbs[jl * (IML+1)+il];
				lnd_ur.x = lndlonbw[jl * (IML+1)+il]; lnd_ur.y = lndlatbn[jl * (IML+1)+il];
	...
	if( pe == root_pe ) {
		...
		for (i=0;i<=IMA;i++) NC_CALL(nc_put_var1_double(ncid, xba_id, &i, &(atmlonbe[i])));
		for (i=0;i<=JMA;i++) NC_CALL(nc_put_var1_double(ncid, yba_id, &i, &(atmlatbs[i])));

		for (i=0;i<IMA;i++) NC_CALL(nc_put_var1_double(ncid, xta_id, &i, &(atmlonc[i])));

		for (i=0;i<JMA;i++) NC_CALL(nc_put_var1_double(ncid, yta_id, &i, &(atmlatc[i])));

		for (i=0;i<=IML;i++) NC_CALL(nc_put_var1_double(ncid, xbl_id, &i, &(lndlonbe[i])));
		for (i=0;i<=JML;i++) NC_CALL(nc_put_var1_double(ncid, ybl_id, &i, &(lndlatbs[i])));

		for (i=0;i<IML;i++) NC_CALL(nc_put_var1_double(ncid, xtl_id, &i, &(lndlonc[i])));

		for (i=0;i<JML;i++) NC_CALL(nc_put_var1_double(ncid, ytl_id, &i, &(lndlatc[i])));
